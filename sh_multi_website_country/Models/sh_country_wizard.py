...
#     @http.route('/set_country', type='json', auth="public", website=True, csrf=False)
...
#         if not country_id:
#             return {'success': False, 'error': 'Missing country_id'}
...
#             ('sh_website_id', '=', website.id)
#         ], limit=1)

#         pricelist = request.env['product.pricelist'].sudo().search([
#             ('sh_country_group_id', '=', country_group.id),
#             ('website_id', '=', website.id)
#         ], limit=1) if country_group else False
...
class CountrySelectionController(http.Controller):
    ...
    def set_country(self, **kwargs):
        ...
        try:
            ...
            country_record = request.env['res.country'].sudo().browse(country_id)
            ...
            if pricelist:
                ...
                # request.website.pricelist = pricelist

                # 💡 Also update the current cart order if exists
            ...
            return request.make_response(json.dumps({
                'success': True,
                # 'flag_url': country_record.image_url
            }), headers=[('Content-Type', 'application/json')])
